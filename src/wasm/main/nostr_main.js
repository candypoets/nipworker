 ...
 */
export function encodeAndPostMessage(worker, message_js) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.encodeAndPostMessage(retptr, addBorrowedObject(worker), addHeapObject(message_js));
 ...
 * @returns {any}
 */
export function decodeWorkerToMainMessage(buffer) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passArray8ToWasm0(buffer, wasm.__wbindgen_export_0);
        const len0 = WASM_VECTOR_LEN;
        wasm.decodeWorkerToMainMessage(retptr, ptr0, len0);
        var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('nostr_main_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
