    ...
    impl RelayTransport for MockRelayTransport {
        ...
        async fn send(&self, url: &str, frame: String) -> Result<(), TransportError> {
            self.calls
                .lock()
                .unwrap()
                .push(Call::Send(url.to_string(), frame.clone()));
    ...
    fn make_writers() -> (
        ...
        let out_writer: Rc<dyn Fn(&str, &str, &str)> =
            Rc::new(move |url: &str, sub_id: &str, raw: &str| {
                out2.lock()
                    .unwrap()
                    .push((url.to_string(), sub_id.to_string(), raw.to_string()));
            });

        let status_writer: Rc<dyn Fn(&str, &str)> = Rc::new(move |st: &str, url: &str| {
            status2
                .lock()
                .unwrap()
                .push((st.to_string(), url.to_string()));
        });

        let to_crypto: Rc<RefCell<dyn Fn(&[u8])>> = Rc::new(RefCell::new(move |bytes: &[u8]| {
        ...
        local
            .run_until(async {
                ...
                assert!(
                    calls
                        .iter()
                        .any(|c| matches!(c, Call::Send(url, f) if url == "wss://r" && f == frame)),
