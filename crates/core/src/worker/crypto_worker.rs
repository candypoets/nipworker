...
fn verify_proof_and_return_y_point(payload: &str) -> Result<String, String> {
    let (proof, keys_map) =
        crate::crypto::utils::parse_verification_payload(payload).map_err(|e| e.to_string())?;
...
fn verify_proof_and_return_y_point(_payload: &str) -> Result<String, String> {
                                    ...
                                    match set_signer_t.signer_type {
                                        ...
                                        fb::SignerTypeT::PrivateKey(pk) => {
                                            match PrivateKeySigner::new(&pk.private_key) {
                                                ...
                                                Err(e) => {
                                                    Err(format!("invalid private key: {}", e))
                                                }
                                            }
                                        }
                                        #[cfg(feature = "crypto")]
                                        fb::SignerTypeT::Nip46Bunker(bunker) => {
                                                ...
                                                .map_err(|e| format!("parse bunker: {}", e))
                                            {
                                                Ok(parsed) => {
                                                    let client_keys =
                                                        bunker.client_secret.as_deref().and_then(
                                                            |s| crate::types::Keys::parse(s).ok(),
                                                        );
                                        ...
                                        fb::SignerTypeT::Nip46QR(qr) => {
                                                ...
                                                .map_err(|e| format!("parse nostrconnect: {}", e))
                                            {
                                                Ok(parsed) => {
                                                    let client_keys =
                                                        qr.client_secret.as_deref().and_then(|s| {
                                                            crate::types::Keys::parse(s).ok()
                                                        });
                ...
                match from_parser.recv().await {
                    Ok(bytes) => {
                        ...
                        let result: Result<String, String> = match req.op() {
                            ...
                            fb::SignerOp::VerifyProof => verify_proof_and_return_y_point(payload),
...
mod new_tests {
    ...
    fn build_signer_request(
        ...
        let sender_off = if sender_pubkey.is_empty() {
            None
        } else {
            Some(builder.create_string(sender_pubkey))
        };
        let recipient_off = if recipient_pubkey.is_empty() {
            None
        } else {
            Some(builder.create_string(recipient_pubkey))
        };
    ...
    async fn test_concurrent_crypto_requests() {
        ...
        local
            .run_until(async {
                ...
                for i in 0..100u64 {
                    let req =
                        build_signer_request(fb::SignerOp::SignEvent, i, "payload", "", "", "");
                    test_parser.send(&req).await.unwrap();
                    sent_ids.push(i);
                }
