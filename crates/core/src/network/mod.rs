...
impl NetworkManager {
    ...
    pub async fn open_subscription(
        ...
        let subscription_manager = SubscriptionManager::new(self.inner.parser.clone());
        ...
        self.inner.subscriptions.lock().unwrap().insert(
            subscription_id,
            Arc::new(futures::lock::Mutex::new(pipeline)),
        );
        Ok(())
    }
    ...
    pub async fn open_subscription_with_default(
        ...
        let to_cache: Arc<dyn MessageSender> = Arc::new(DummyMessageSender);
        let pipeline =
            Pipeline::default(self.inner.parser.clone(), to_cache, subscription_id.clone())
    ...
    pub async fn publish_event(
        ...
        self.inner
            .publish_manager
            .publish_event(publish_id, template)
            .await
    }
