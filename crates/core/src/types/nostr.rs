...
impl SecretKey {
    ...
    pub fn public_key_from_secret(&self) -> PublicKey {
        ...
        let signing_key =
            SigningKey::from_bytes(&self.0).expect("Secret key must be valid 32-byte scalar");
    ...
    pub fn from_json(json: &str) -> Result<Self> {
        ...
        let content = {
            if let Some(i) = json.find("\"content\"") {
                ...
                String::from_utf8(raw_bytes).map_err(|_| {
                    TypesError::InvalidFormat("Invalid UTF-8 in content".to_string())
                })?
            } else {
...
impl Event {
    ...
    pub fn to_json(&self) -> String {
        let tags_json = NostrTags(self.tags.clone()).to_json();
        let mut result = String::with_capacity(64 + self.content.len() * 2 + tags_json.len() + 20);
        use core::fmt::Write;
        write!(
			result,
            ...
            if let Some(i) = json.find("\"pubkey\"") {
                ...
                if let Some(q1) = tail.find('"') {
                    ...
                    if let Some(q2) = tail[pk_start..].find('"') {
                        ...
                        return Err(TypesError::InvalidFormat(
                            "Missing pubkey value".to_string(),
                        ));
                    }
                } else {
                    return Err(TypesError::InvalidFormat(
...
impl RelayMessage {
    pub fn new(message_type: String, data: String) -> Self {
        RelayMessage { message_type, data }
    }
}
