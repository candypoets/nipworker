...
use crate::types::{Event, REPOST, TEXT_NOTE};
...
fn build_nested_parsed_event<'a, A: flatbuffers::Allocator + 'a>(
    ...
    for tag in &event.event.tags {
        ...
        let string_vec = fb::StringVec::create(builder, &fb::StringVecArgs { items: Some(vec) });
...
fn build_nested_parsed_data<'a, A: flatbuffers::Allocator + 'a>(
    ...
    match data {
        ...
        _ => Err(ParserError::Other(format!(
            "Unsupported nested parsed data type for repost: not Kind1"
        ))),
    }
}
