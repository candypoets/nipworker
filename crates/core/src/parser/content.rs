    ...
    match nip19::FromBech32::from_bech32(entity_str) {
        Ok(decoded) => {
            let (prefix, id, relays, author, kind) = match decoded {
                ...
                Nip19::EventId(note) => ("note", note.to_string(), Vec::new(), None, None),
...
fn process_emoji_with_map(
    ...
    if let Some((url, _emoji_set)) = emoji_map.get(shortcode) {
        Ok(
            ContentBlock::new("emoji".to_string(), text.to_string()).with_data(
                ContentData::Emoji {
...
pub fn serialize_content_data<'a, A: flatbuffers::Allocator + 'a>(
    ...
    match data {
        ...
        ContentData::Nostr {
            ...
            let relays_off = if relays_strs.is_empty() {
                None
            } else {
                Some(builder.create_vector(&relays_strs))
            };
        ...
        ContentData::MediaGrid { items } => {
            ...
            let grid_fb = fb::MediaGroupData::create(
                builder,
