...
async fn drain_crypto_ack(event_sink_rx: &mut futures::channel::mpsc::Receiver<(String, Vec<u8>)>) {
    ...
    while tokio::time::Instant::now() < deadline {
        match tokio::time::timeout(Duration::from_millis(200), event_sink_rx.next()).await {
            Ok(Some((sub_id, bytes))) if sub_id == subscription_id => {
                ...
                let wm =
                    flatbuffers::root::<fb::WorkerMessage>(leaked).expect("valid WorkerMessage");
