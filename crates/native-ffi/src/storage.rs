...
impl InMemoryStorage {
	...
	fn extract_tags_from_fb(
		tags_fb: flatbuffers::Vector<flatbuffers::ForwardsUOffset<nipworker_core::generated::nostr::fb::StringVec>>,
	) -> Vec<Vec<String>> {
	...
	fn event_matches_filter(
		...
		if let Some(ref e_tags) = filter.e_tags {
			if !tags.iter().any(|t| t.len() >= 2 && t[0] == "e" && e_tags.contains(&t[1])) {
				return false;
			}
		}
		...
		if let Some(ref p_tags) = filter.p_tags {
			if !tags.iter().any(|t| t.len() >= 2 && t[0] == "p" && p_tags.contains(&t[1])) {
				return false;
			}
		}
		...
		if let Some(ref a_tags) = filter.a_tags {
			if !tags.iter().any(|t| t.len() >= 2 && t[0] == "a" && a_tags.contains(&t[1])) {
				return false;
			}
		}
		...
		if let Some(ref d_tags) = filter.d_tags {
			if !tags.iter().any(|t| t.len() >= 2 && t[0] == "d" && d_tags.contains(&t[1])) {
				return false;
			}
		}
		...
		for (tag_key, filter_values) in &filter.tags {
			if !tags.iter().any(|t| t.len() >= 2 && &t[0] == tag_key && filter_values.contains(&t[1])) {
				return false;
			}
		}
...
mod tests {
	...
	async fn test_inmemory_storage_q_tag_filter() {
		...
		let mut filter = Filter::new();
		filter.tags.insert("q".to_string(), vec![quoted_id.to_string()]);
		let results = storage.query(vec![filter]).await.unwrap();
		assert_eq!(results.len(), 1, "Should return exactly one event with matching q tag");
	...
	async fn test_inmemory_storage_e_tag_filter() {
		...
		let with_e = build_worker_message_bytes_with_tags(
			...
			"event_with_e",
			1000,
			vec![vec!["e".to_string(), e_id.to_string(), "wss://relay.example.com".to_string()]],
		);
		storage.persist(&with_e).await.unwrap();

		let without_e = build_worker_message_bytes("save_to_db", 1, pubkey, "event_without_e", 1000);
		storage.persist(&without_e).await.unwrap();

		let mut filter = Filter::new();
		filter.e_tags = Some(vec![e_id.to_string()]);
		let results = storage.query(vec![filter]).await.unwrap();
		assert_eq!(results.len(), 1, "Should return exactly one event with matching e tag");
	...
	async fn test_inmemory_storage_uppercase_nip22_tag_filter() {
		...
		let nip22_event = build_worker_message_bytes_with_tags(
			...
			vec![
				...
				vec!["e".to_string(), parent_id.to_string(), "wss://relay.example.com".to_string()],
			],
		);
		...
		let mut filter = Filter::new();
		filter.tags.insert("E".to_string(), vec![parent_id.to_string()]);
		let results = storage.query(vec![filter]).await.unwrap();
		assert_eq!(results.len(), 1, "Uppercase #E filter should match only NIP-22 event");
		let (_, _, id, _, _) = InMemoryStorage::extract_event_fields(&results[0]).unwrap();
		assert_eq!(id, "nip22_comment");

		// Query for uppercase #P -> same
		let mut filter = Filter::new();
		filter.tags.insert("P".to_string(), vec![parent_pubkey.to_string()]);
		let results = storage.query(vec![filter]).await.unwrap();
		assert_eq!(results.len(), 1);
		...
		let results = storage.query(vec![filter]).await.unwrap();
		assert_eq!(results.len(), 1, "Lowercase #e should match only the event with lowercase e tag");
		let (_, _, id, _, _) = InMemoryStorage::extract_event_fields(&results[0]).unwrap();
		assert_eq!(id, "nip22_comment");
	}
}
